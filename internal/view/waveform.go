...
func RenderWaveforms(m model.Model) string {
	if m.TwoLineMode {
		if m.SelectMode {
			return renderSelectModeWaveformsTwoLine(m)
		}
		return renderNormalModeWaveformsTwoLine(m)
	}
...
// renderNormalModeWaveformsTwoLine renders waveforms in normal mode (2-line per signal)
func renderNormalModeWaveformsTwoLine(m model.Model) string {
	var lines []string
	visibleCount := m.VisibleSignalCount()
	width := m.WaveformWidth()
	indices := m.VisibleSignalIndices()
...
// renderSelectModeWaveformsTwoLine renders waveforms in select mode (2-line per signal)
func renderSelectModeWaveformsTwoLine(m model.Model) string {
	var lines []string
	visibleCount := m.VisibleSignalCount()
	width := m.WaveformWidth()
