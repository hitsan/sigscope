...
func RenderSignalList(m model.Model) string {
	if m.TwoLineMode {
		if m.SelectMode {
			return renderSelectModeListTwoLine(m)
		}
		return renderNormalModeListTwoLine(m)
	}
...
// renderNormalModeListTwoLine renders signal list in normal mode (2-line per signal)
func renderNormalModeListTwoLine(m model.Model) string {
	var lines []string
	visibleCount := m.VisibleSignalCount()
	indices := m.VisibleSignalIndices()
...
// renderSelectModeListTwoLine renders signal list in select mode (2-line per signal)
func renderSelectModeListTwoLine(m model.Model) string {
	var lines []string
	visibleCount := m.VisibleSignalCount()
